...
void save_passenger(passengerList *PList)
{
    ...
    fprintf(fp,"姓名：  %s：证件号：  %s：航班信息：  %s订单号：  %d订票数量：  %d\n",
            temp->name,temp->ID_num,temp->flight_num,temp->order_num,temp->ticket_num);
        temp = temp->next;
    };
    fclose(fp);
}
void load_passenger(passengerList *PList)
{
    ...
    while(!feof(fp))
    {
      ...
      //n =fread(q,sizeof(passengernode),1,fp);
       n=fscanf(fp,"姓名：  %s：证件号：  %s：航班信息：  %s订单号：  %d订票数量：  %d\n",
                q->name,q->ID_num,q->flight_num,q->order_num,q->ticket_num);
        if(n!=1)
            break;
        PList->rear->next=q;
        PList->rear=q;
...
void save_flight(flightnode *h)
{
    ...
    while(temp!=NULL)
    {
     fprintf(fp,"出发时间：   %s 抵达时间：   %s 出发地点：   %s 抵达地点：   %s\n",
     temp->start_time,temp->end_time,temp->start_place,temp->end_place);
...
void load_flight(flightnode *&h)
{
    ...
    while(!feof(fp))
    {
       ...
       n=fscanf(fp,"出发时间：    %s 抵达时间：    %s 出发地点：    %s 抵达地点：    %s\n",
                q->start_time,q->end_time,q->start_place,q->end_place);
        if(n!=1)
            break;
        pt->next=q;
        pt=q;
